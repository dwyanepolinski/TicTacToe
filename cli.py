...
def host_game(client,nick):
	...
	msg=client.recv(2048).strip()
	play(msg,client,nick,'')

def list_games(client):
	client.send('LIST')
	glist=client.recv(2048).strip('@').split('|')[1:]
...
def move(client,opponent,zn):
	...
	while True:
		...
		if position.isdigit() and int(position)-1 in range(9) and board[int(position)-1].isdigit():
			...
			print 'Źle podana pozycja, spróbuj jeszcze raz'
...
def sync(client,opponent,zn):
	...
	if msg=='STOP':
		...
		printstatus(True,nick,opponent,('Gratulacje, wygrałeś!' if msg=='WIN' else 'Remis!' if msg=='DRAW' else 'Niestety, przegrałeś'))
		show()
		for i in range(9):
			board[i]=str(i+1)
...
def play(msg,client,nick,opponent):
	...
	printstatus(True,nick,opponent,'Rozpoczęto grę, '+('zaczynasz' if zn=='x' else 'przeciwnik zaczyna'))
	while True:
		if s:
...
def start(client,error):
	inp=raw_input('Zakładasz grę czy dołączasz do gry? \n(\'tak\' - zakładam/Enter - nie zakładam/\':\' - wyjscie) :')
	if inp=='tak':
		...
		while True:
			...
			while True:
				list_games(client)
				opponent=raw_input('Podaj nik gracza z jakim chcesz zagrać, listuj gry ponownie lub załóż grę \n(podaj nick/\'@\' - listuj/załóż - Enter/\'w\' - wyjście) :')
				if inp==':':
					client.send('EXIT')
					sys.exit('Pomyślnie rozłączono z serwerem. Bye') 
				if opponent!='@':
